...
bool Board::doMove() {
    ...
    while (!stop)
    {
        (turn == WHITE) ? cout << "RUCH BIAŁYCH" << endl : cout << "RUCH CZARNYCH" << endl;
        cout << "Wpisz koordynaty ruchu jaki chcesz wykonać zaczynając od początkowego x i y, a kończąc na docelowym x i y (np. 0102)" << endl;
        cin >> move;
        x1 = move[0] - 48;
        y1 = move[1] - 48;
        x2 = move[2] - 48;
    ...
    if (getSquare(x2, y2)->getPiece() == KING)
        ...
        {
            cout << "BIALI WYGRALI" << endl;
            return false;
        }
        else

        {
            cout << "CZARNI WYGRALI" << endl;
            return false;
        }
...
bool Board::makeMove(int x1, int y1, int x2, int y2) {
    ...
    {
        std::cout << "Błąd. Koordynaty spoza planszy" << std::endl;
        return false;
    }
    ...
    {
        std::cout << "Niepoprawny ruch. Nie można zastąpić swojego pionka" << std::endl;
        return false;
    }
    ...
    switch (src->getPiece())
    {
        ...
        case PAWN: return movePawn(src, dest);
            break;
        case EMPTY: std::cout << "Nie masz w tym miejscu pionka" << std::endl;  return false;
            break;
        default: std::cerr << "Wystąpił błąd. Spróbuj uruchomić ponownie grę" << std::endl;
            break;
    }
    return false;
}
void Board::saveGame(const std::string& filename)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            file << square[i][j].getPiece() << " " << square[i][j].getColor() << " ";
        }
        file << "\n";
    }
    file << turn;
...
void Board::loadGame(const std::string& filename)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            int piece, color;
            file >> piece >> color;
            square[i][j].setPieceAndColor(static_cast<Piece>(piece), static_cast<Color>(color));
        }
    }
    int turnColor;
    file >> turnColor;
